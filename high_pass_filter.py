...
def high_pass_filter():
    ...
    if file is not None:
        ...
        st.sidebar.header("Apply High Pass Filter")

        # Button to apply the high-pass filter
        if st.sidebar.button("Apply High Pass Filter"):
            ...
            radius = 30  # High-pass filter radius

            mask = np.ones((rows, cols), np.float32)
            mask[crow - radius:crow + radius, ccol - radius:ccol + radius] = 0  # Block low frequencies
